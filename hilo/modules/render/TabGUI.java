...
import hilo.settings.*;
import hilo.settings.Setting;
...
public class TabGUI extends Module {
	...
	public void onEvent(Event e) {
			if (e instanceof EventRenderGUI) {
				int primaryColor = 0xff0077FF, secondaryColor = 0xff0067AA;
		
				Gui.drawRect(5, 30, 70, 30 + Module.Category.values().length * 16 + 1, 0x90000000);
				Gui.drawRect(5, 30 + currentTab * 16, 7 + 61 + 2, 33 + currentTab * 16 + 12 + 2, 0xff880088);
				...
				if (expanded) {
					...
					List<Module> modules = Hilo.getModuleByCategory(category);
					
					if (modules.size() == 0)
						return;
					
					Gui.drawRect(70, 30, 70 + 68, 30 + modules.size() * 16 + 1, 0x99000000);
					Gui.drawRect(70, 30 + category.moduleIndex * 16, 7 + 61 + 70, 33 + category.moduleIndex * 16 + 12 + 2, 0xff0077FF);
					...
					for (Module m : modules) {
						...
						if (count == category.moduleIndex && m.expanded) {
							...
							if (!m.settings.isEmpty()) {
								Gui.drawRect(70 + 68, 30, 70 + 68 + maxLength + 9, 30 + m.settings.size() * 16 + 1, 0x99000000);
								Gui.drawRect(70 + 68, 30 + m.index * 16, 7 + 61 + maxLength + 9 + 70, 33 + m.index * 16 + 12 + 2, m.settings.get(m.index).focused ? secondaryColor : 0xff0077FF);
