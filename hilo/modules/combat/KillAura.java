...
import hilo.events.listeners.EventMotion;
import hilo.events.listeners.EventUpdate;
import hilo.modules.Module;
import hilo.settings.BooleanSetting;
import hilo.settings.ModeSetting;
...
public class KillAura extends Module {
	...
	public NumberSetting range = new NumberSetting("Range", 4, 1, 10, 0.1);
	...
	public void onEvent(Event e) {
		if (e instanceof EventMotion) {
			if (e.isPre()) {
				...
				List<Entity> targets = (List<Entity>) mc.theWorld.loadedEntityList.stream().filter(Entity.class::isInstance).collect(Collectors.toList());
				...
				if (!targets.isEmpty()) {
					...
					event.setYaw(getRotation(target)[0]);
					event.setPitch(getRotation(target)[1]	);
